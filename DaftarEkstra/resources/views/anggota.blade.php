    ...
    <div class="content-header" style="background-color: white; margin-bottom: 0.5rem">
        <div class="container-fluid">
            <div class="row">
                ...
                <div class="col-sm-6">
                    <a class="btn btn-primary" style="float: right" href="{{ url('/anggota/create') }}">Create</a>
                </div><!-- /.col -->
            </div><!-- /.row -->
        </div><!-- /.container-fluid -->
    </div>
                    ...
                    <table id="example1" class="table table-striped table-bordered">
                        ...
                        @foreach($anggota as $key=>$a)
                            <tr>
                                ...
                                @if($ekstra[$key]->id == $a->id_ekstra)
                                    <td>{{ $ekstra[$key]->nama }}</td>
                                @endif

                                <td>
                                    <a class="btn btn-warning" href="{{ url('/anggota/edit/'.$a->id) }}">
                                        <i class="nav-icon fas fa-pen"></i>
                                    </a>
                                    <a class="btn btn-danger" data-toggle="modal" data-target="#exampleModal" onclick="anggotaID = {{ $a->id }}">
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-primary" onclick="deleteAnggota()">Iya, hapus data</button>
            </div>
        </div>
    </div>
</div>
